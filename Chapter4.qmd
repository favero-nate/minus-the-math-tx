...
## Using Confidence Intervals[^chapter4-4]
...
where $μ_f$ is the population mean for females and $μ_m$ is the population mean for males. Because the difference in these population means is the main parameter we wish to estimate, we say the confidence interval for the parameter of interest is [0.29, 2.65]. This analysis provides evidence that the mean for females is higher than the mean for males, and that the difference between means in the population is likely to be between 0.29 and 2.65. Since 0 does not fall within the range of the confidence interval, we’ve found evidence that there *is* a difference between males and females, so we can say that the results are **statistically significant**.
