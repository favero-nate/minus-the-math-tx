...
## Confidence Intervals[^estimation-2]

[^estimation-2]: This section is adapted from David M. Lane. “Confidence Intervals Introduction.” *Online Statistics Education: A Multimedia Course of Study*. <http://onlinestatbook.com/2/estimation/confidence.html>
...
It is natural to interpret a 95% confidence interval as an interval with a 0.95 probability of containing the population mean. However, the proper interpretation is not that simple (as will be explained more fully in @sec-interpreting-confidence-intervals-correctly). One problem is that the computation of a confidence interval does not take into account any other information you might have about the value of the population mean. For example, if numerous prior studies had all found sample means above 110, it would not make sense to conclude that there is a 0.95 probability that the population mean is between 72.85 and 107.15.

Confidence intervals can be computed for various **parameters**,[^estimation-3] not just the mean. For example, it is common to compute a confidence interval for $\rho$, the population value of Pearson's r, based on sample data.

[^estimation-3]: A parameter is a value calculated in a population. For example, the mean of the numbers in a population is a parameter. Compare with a sample statistic, which is a value computed in a sample to estimate a parameter. (<https://onlinestatbook.com/2/glossary/parameter.html>)

## Using Confidence Intervals[^estimation-4] {#sec-using-confidence-intervals}

[^estimation-4]: This section is adapted from David M. Lane. “Difference between Means.” *Online Statistics Education: A Multimedia Course of Study*. <http://onlinestatbook.com/2/estimation/difference_means.html>

It is much more common for a researcher to be interested in the difference between means than in the specific values of the means themselves. Using statistical jargon introduced in the prior section, we could therefore say that the *parameter of interest* is often the difference in population means. We take as an example the data from the "Animal Research"[^estimation-5] case study. In this experiment, students rated (on a 7-point scale) whether they thought animal research is wrong. The sample sizes, means, and variances are shown separately for males and females in @tbl-animalresearchmeanvar.

[^estimation-5]: <http://onlinestatbook.com/2/case_studies/animal_research.html>
...
## Introducing Confidence Intervals for Regression[^estimation-6] {#sec-introducing-confidence-intervals-for-regression}

[^estimation-6]: This section is written by Nathan Favero.

Let's examine one more practical example of using confidence intervals, this time in the context of regression. We will again consider the regression results table presented at the end of the previous chapter (@sec-quick-guide-to-interpreting-regression-results). The table is shown again here as @tbl-regression-cs-gpa-repeated.
...
Because the coefficients we see in the table are just estimates, confidence intervals can help us better understand the precision of these estimates by providing us with a range of plausible values for the coefficients. Notice that no confidence intervals have been provided in the table (which is a situation you may frequently encounter when reading social scientific research publications). Fortunately, we can easily obtain a good approximation of a 95% confidence interval for coefficient estimate $\hat{\beta_i}$ using the standard error estimate ($s_{\beta_i}$) provided in the table, along with the following formulas:
...
Again, these formulas are an approximation; the formulas for precise intervals are shown in @sec-confidence-interval-for-a-regression-slope-coefficient.[^estimation-7] The approximation provides an inexact but close approximation of a 95% confidence interval as long as the sample size (e.g., at least 30 more observations than the number of independent variables included in the regression). In this case, there are 105 observations and only two independent variables, so we will obtain a good approximation. And an approximation is usually the best we can hope for when calculating confidence intervals by hand from a regression table, since we will usually also lose some precision due to rounding error.

[^estimation-7]: Specifically, two is the approximate value by which the standard error should be multiplied, but a more exact value can be found using the t distribution, as explained in @sec-sampling-distributions.

For the verbal SAT scores, we find the following limits:

$$ \text{Lower limit} \approx 0.0017 - (2)(0.0010) = -0.0003 $$

$$ \text{Upper limit} \approx 0.0017 + (2)(0.0010) = 0.0037 $$
...
$$ \text{Lower limit} \approx 0.0048 - (2)(0.0012) = 0.0024 $$

$$ \text{Upper limit} \approx 0.0048 + (2)(0.0012) = 0.0072 $$
...
## Interpreting Confidence Intervals Correctly[^estimation-8] {#sec-interpreting-confidence-intervals-correctly}

[^estimation-8]: This section is written by Nathan Favero.

While confidence intervals are very useful, it is difficult to provide a technically-accurate interpretation of one.[^estimation-9] That is because the "% confidence" in a "95% confidence interval for the mean" refers to the accuracy of the *process* of creating a confidence interval—not the probability that a specific confidence interval we encounter will contain the true value of the population mean. If this distinction seems confusing, it is!

[^estimation-9]: Stephens, M. (2023). The Bayesian lens and Bayesian blinkers. *Philosophical Transactions of the Royal Society A*, *381*(2247), 20220144.
...
Fortunately, even if you miss the precise details, you will still probably get something useful out of confidence intervals.[^estimation-10] Nonetheless, let's try to set the record straight.

[^estimation-10]: Anderson, A. A. (2019). Assessing statistical results: magnitude, precision, and model uncertainty. *The American Statistician*, *73*(sup1), 118-121.

An analogy may help. Suppose you are interacting with a chatbot that is truthful 95% of the time and lies the other 5%.[^estimation-11] For each statement, will you always conclude it has a 95% chance of being true? Not necessarily. If the chatbot discusses a topic you already know a lot about, you will probably be able to pick out the lies from the true statements with fairly high confidence. Some things the bot says will be things you know to be true, so you can be nearly 100% sure they are true. Other statements will be things you're quite sure are wrong, so you will conclude that the probability they are true is close to 0%. If you wanted to be very systematic, you could even use the mathematical formula known as Bayes' theorem[^estimation-12] to combine your prior knowledge of a statement's probability of being true with the fact that a 95%-accurate bot claimed the statement was true, allowing you to precisely quantify how confident you should be about the statement's truth in the end. 

[^estimation-11]: This example is adapted from Behar, R., Grima, P., & Marco-Almagro, L. (2013). Twenty-five analogies for explaining statistical concepts. *The American Statistician*, *67*(1), 44-48.

[^estimation-12]: See <https://onlinestatbook.com/2/glossary/bayes.html> or <https://onlinestatbook.com/2/probability/bayes_demo.html>.
...
In the same way, it turns out that *absent any other information,* a 95% confidence interval is often a good approximation for a range of values that contains the population parameter with 95% probability.[^estimation-13] Thus, I think it is quite reasonable that many of us, when we see a mean estimate with a 95% confidence interval ranging from A to B, assume there is a 95% chance the population mean does indeed lie between A and B. But technically, that is not a direct interpretation of the confidence interval; instead, this statement about plausible values of the population mean is a subjective conclusion that I can draw based on the confidence interval. Another person might see the same confidence interval and reasonably decide—drawing on their own prior knowledge of the topic—that the confidence interval contains values that are highly implausible, and thus they would reach a different conclusion from me about how likely the interval is to contain the true population mean. 

[^estimation-13]: Kass, R. E. (2011). Statistical inference: The big picture. *Statistical science: a review journal of the Institute of Mathematical Statistics*, *26*(1), 1.
