...
[^comparing-means-1]: This section is adapted from David M. Lane. “Difference between Two Means (Independent Groups).” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/tests_of_means/difference_means.html>
...
[^comparing-means-2]: <https://onlinestatbook.com/2/case_studies/animal_research.html>
...
3.  Each value is sampled independently[^comparing-means-3] from each other value. This assumption requires that each subject provide only one value. If a subject provides two scores, then the scores are not independent.

[^comparing-means-3]: Two variables are said to be independent if the value of one variable provides no information about the value of the other variable. In this case, if knowing the value of the X variable for one observation could help us predict the value of X for another observation, the two values of X are not independent. For example, if there is clustered sampling, such that selecting an individual into the sample implies that neighbors with similar X values are also likely to be in the sample, the observations are not independent.
...
The next step is to compute the estimate of the standard error of the statistic. In this case, the statistic is the difference between means, so the estimated standard error of the statistic is $s_{\text{diff}}$ (which can also be written as $s_{\bar{X}_1 - \bar{X}_2}$). The formula for the standard error of the difference between means is:[^comparing-means-4]

[^comparing-means-4]: For a more detailed discussion, see <https://onlinestatbook.com/2/sampling_distributions/samplingdist_diff_means.html>
...
Once we have the degrees of freedom, we can use a t distribution calculator[^comparing-means-5] to find the probability. @fig-2tailprob shows that the probability value (p) for a two-tailed test is 0.0164. The two-tailed test is used when the null hypothesis can be rejected regardless of the direction of the effect. As shown in @fig-2tailprob, it is the probability of a t \< -2.533 or a t \> 2.533.

[^comparing-means-5]: <https://onlinestatbook.com/2/calculators/t_dist.html>
...
The problem with this approach is that if you did this analysis, you would have six chances to make a Type I error. Therefore, if you were using the 0.05 significance level, the probability that you would make a Type I error on at least one of these comparisons is greater than 0.05.[^comparing-means-8] The more means that are compared, the more the Type I error rate is inflated. @fig-pairwisecompfmean shows the number of possible comparisons between pairs of means (pairwise comparisons) as a function of the number of means. If there are only two means, then only one comparison can be made. If there are 12 means, then there are 66 possible comparisons.

[^comparing-means-8]: When discussing probability of Type I errors, we assume all null hypotheses are true, since a Type I error can't occur if the null hypothesis is false.

![Number of pairwise comparisons as a function of the number of means.](Images/comparing-means/pairwisecompfmean.png){#fig-pairwisecompfmean width="400"}
...
![Probability of a Type I error as a function of the number of means.](Images/comparing-means/type1error.png){#fig-type1error width="400"}
...
4.  Compute p for each comparison using a Studentized Range Calculator.[^comparing-means-9] The degrees of freedom is equal to the total number of observations minus the number of means. For this experiment, df = 136 - 4 = 132.

[^comparing-means-9]: <https://onlinestatbook.com/2/calculators/studentized_range_dist.html>
...
### Introduction[^comparing-means-10]

[^comparing-means-10]: This subsection is adapted from David M. Lane. “Introduction.” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/analysis_of_variance/intro.html>
...
### The Critical Step: Calculating an F Ratio[^comparing-means-11] {#sec-the-critical-step-calculating-an-f-ratio}

[^comparing-means-11]: This subsection and the following are adapted from David M. Lane. “One-Factor ANOVA (Between Subjects).” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/analysis_of_variance/one-way.html>
...
![Distribution of F.](Images/comparing-means/fdist_smiles.png){#fig-fdist width="375"}
...
An F distribution calculator[^comparing-means-12] shows that p = 0.018. Again, because this value is less than 0.05, one would generally reject the null hypothesis and conclude that average leniency varies depending on type of smile. The p-value from an ANOVA is sometimes reported in a larger table of summary results such as @tbl-anova-results-summary.

[^comparing-means-12]: <https://onlinestatbook.com/2/calculators/F_dist.html>
...
### Terminology for Various Designs[^comparing-means-13] {.unnumbered}

[^comparing-means-13]: This subsection is adapted from David M. Lane. “Analysis of Variance Designs.” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/analysis_of_variance/anova_designs.html>
...
### Details of One-Factor ANOVA (Between Subjects)[^comparing-means-15] {.unnumbered}

[^comparing-means-15]: This subsection is adapted from David M. Lane. “One-Factor ANOVA (Between Subjects).” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/analysis_of_variance/one-way.html>
